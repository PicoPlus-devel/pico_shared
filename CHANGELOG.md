...
- **Controller Test now shows the detected pad type and, for the GPIO ports, the raw word the pad shifted out** (`Sent by pad: 0002 hex`), taken before any NES/SNES masking. This tells a button that never reaches the Pico apart from one that is decoded wrong — which is what identified the adapter cable above as an active converter rather than a passive rewire. New `nespad_raw_ext[]` in the driver carries it.
- **Fix: menu buttons on a SNES pad wired to a GPIO port.** The menu OR-ed in only the 8-bit `nespad_states[]`, which holds a SNES pad's *first eight* serial bits — B and Y sit where a NES pad has A and B. So "choose" landed on B, "back" on Y, and A did nothing at all, while X could not open the recently played list the way it does on USB and Wii Classic pads. The four face buttons of a SNES pad are now named rather than taken positionally: **A chooses, B goes back**, X opens the recently played list, and Y is unused, exactly as on the other pad types. NES pads are untouched (their word is already in menu order), and so is an 8-bit SNES->NES adapter cable, which reports NES buttons in NES order. Emulators are free to name the same four buttons in their own game path; pico-infonesPlus does so as of the same release, so choosing a ROM and playing it use the same two buttons.
- **The 16-clock read is now confirmed against genuine SNES hardware**: a SNES controller port wired straight to the NES port GPIOs delivers all 12 buttons, L and R included. Adapter *cables* are the thing to watch out for — several contain a converter, sometimes moulded into the plug, and then only 8 buttons can ever arrive.
